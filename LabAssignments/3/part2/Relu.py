...
def forward_propogation(parameters,input):
	...
	for i in range(len(parameters)-1):
		X = relu(X@(parameters[i][0].T) + parameters[i][1])
	X = sigmoid(X@(parameters[len(parameters)-1][0].T) + parameters[len(parameters)-1][1])
	return(X)
	...
	for epoch in range(epochs):
		for i in range(iters):
			...
			for j in range(len(parameters) - 1):
				X[j+1] = relu(X[j]@(parameters[j][0].T) + parameters[j][1])
			X[len(parameters)] = sigmoid(X[len(parameters)-1]@(parameters[len(parameters)-1][0].T) + parameters[len(parameters)-1][1])
			...
			for k in range(len(parameters)-2,-1,-1):
				O[k] = relu_derivative(X[k+1]) #X is shifted because X[0] have the inputs
		...
		t_cost = cost(parameters,features,labels)
		# print("J  :",t_cost)
		# print("J_prev :",t_prev)
		...
		if diff < tol:
			...
			if flag == 10:
				...
				if learning_rate*batch_size <1e-12:
